...
RSpec.describe TLAW::Params::Param do
  ...
  describe '#call' do
    ...
    context 'typechecking' do
      ...
      its_call('5', type: Integer) {
        is_expected.to raise_error TypeError, 'expected instance of Integer, got "5"'
      }
      its_call(Time.parse('2018-03-01'), type: :year) {
        is_expected.to ret(x: '2018')
      }
      its_call('2018-03-01', type: :year) {
        is_expected.to raise_error TypeError, 'expected object responding to #year, got "2018-03-01"'
      }
    end
